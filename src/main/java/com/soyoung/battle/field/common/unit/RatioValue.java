        ...
        if (sValue.endsWith("%")) {
            ...
            try {
                ...
                if (percent < 0 || percent > 100) {
                    throw new BattlefieldParseException("Percentage should be in [0-100], got [{}]", percentAsString);
                }
                return new RatioValue(Math.abs(percent));
            } catch (NumberFormatException e) {
                throw new BattlefieldParseException("Failed to parse [{}] as a double", e, percentAsString);
            }
        } else {
            try {
                double ratio = Double.parseDouble(sValue);
                if (ratio < 0 || ratio > 1.0) {
                    throw new BattlefieldParseException("Ratio should be in [0-1.0], got [{}]", ratio);
                }
                return new RatioValue(100.0 * Math.abs(ratio));
            } catch (NumberFormatException e) {
                throw new BattlefieldParseException("Invalid ratio or percentage [{}]", sValue);
            }
